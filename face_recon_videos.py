...
class VideoPathDataset(torch.utils.data.Dataset):
    def __init__(self, filenames, txt_filenames, bfm_folder, output_dir):
    ...
    def __getitem__(self, index):
        ...
        fname = os.path.join(self.output_dir, name[-2], name[-1])
...
def main(opt, model):
    ...
    dataset = VideoPathDataset(filenames, keypoint_filenames, opt.bfm_folder, opt.output_dir)
    ...
    for data in tqdm(dataloader):
        ...
        if data['filename'][0] == "skip":
            continue           


        num_batch = data['imgs'][0].shape[0] // batch_size

        if data['imgs'][0].shape[0] % batch_size > 0:
            num_batch = num_batch + 1
        
        print("process ", data['filename'][0], " batch ", num_batch, " img ", data['imgs'][0].shape[0])
